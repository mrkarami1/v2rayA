...
var ServiceNameList = []string{"sing-box", "xray", "v2ray"}
...
func GetV2rayServiceVersion() (variant Variant, ver string, err error) {
	...
	if envConfig.V2rayBin != "" && envConfig.CoreType == "" {
		return Unknown, "", fmt.Errorf("when using custom v2ray-bin path, you must specify --core-type (v2ray, xray, or sing-box) or set V2RAYA_CORE_TYPE environment variable")
	}
	...
	if envConfig.CoreType != "" {
		...
		case "sing-box", "singbox":
			variant = Singbox
		default:
			return Unknown, "", fmt.Errorf("invalid core type '%s', must be 'v2ray', 'xray', or 'sing-box'", envConfig.CoreType)
		}
	} else {
	...
	if fields = strings.Fields(strings.TrimSpace(output.String())); len(fields) < 2 {
		return Unknown, "", fmt.Errorf("cannot parse version of core")
	}
	ver = fields[1]
